...
class playmusicmodule(commands.Cog, name="PlayMusicModule", description=""):
    ...
    async def leave(self, ctx):
        ...
        if voice.is_connected():
            voice.stop()
            music_helper.delete_songs_list()
    ...
    async def play(self, ctx, *, yt_search_or_link: Optional[str]):
        if yt_search_or_link == None:
            ...
            if not channel:
                ...
                yt_json = music_helper.add_url(yt_search_or_link)
                print(yt_json)
                ...
                if voice and voice.is_connected():
                    ...
                    await channel.connect()
                if not music_helper.check_duration(yt_json):
                    await ctx.send('Sorry! The song you\'re trying to play is too long! :cry: ')
                else:
